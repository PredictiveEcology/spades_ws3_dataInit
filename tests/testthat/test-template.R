...
#    `functionName` is a function's name in your module (e.g., `dataInitEvent1`).
# 2. Copy this file to the tests folder (i.e., `/media/data/gparadis/dev_/fiim/modules/dataInit/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("dataInit")
  path <- list(modulePath = "/media/data/gparadis/dev_/fiim/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    dataInit = list(.saveInitialTime = NA)
  )
  ...
  if (exists("dataInitEvent1", envir = .GlobalEnv)) {
    simOutput <- dataInitEvent1(mySim)
  } else {
  ...
  if (exists("dataInitEvent2", envir = .GlobalEnv)) {
    simOutput <- dataInitEvent2(mySim)
  } else {
